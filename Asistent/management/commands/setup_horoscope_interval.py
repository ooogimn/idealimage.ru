"""
Команда для настройки интервального автопостинга гороскопов
Создает 12 расписаний: каждые 15 минут с 8:00 до 10:45 (по одному знаку за раз)

⚠️ ВНИМАНИЕ: Это для другого сценария использования!
- Эта команда создает 12 отдельных расписаний (каждые 15 минут по одному знаку)
- Для генерации всех 12 гороскопов сразу в 10:00 используйте: setup_horoscope_daily_10am.py
...
class Command(BaseCommand):
    help = 'Настройка интервального автопостинга гороскопов: каждые 15 минут с 8:00, 12 раз в день (по одному знаку за раз). Для генерации всех 12 сразу в 10:00 используйте setup_horoscope_daily_10am'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('  🔮 НАСТРОЙКА ИНТЕРВАЛЬНОГО АВТОПОСТИНГА ГОРОСКОПОВ'))
        self.stdout.write(self.style.SUCCESS('=' * 70))
        self.stdout.write('')
        self.stdout.write(self.style.WARNING(
            '⚠️ ВНИМАНИЕ: Эта команда создает 12 расписаний (каждые 15 минут по одному знаку)'
        ))
        self.stdout.write(self.style.WARNING(
            '   Для генерации всех 12 гороскопов сразу в 10:00 используйте: setup_horoscope_daily_10am'
        ))
        self.stdout.write('')
