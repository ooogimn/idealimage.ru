"""
Management команда для переноса статей из категории "ЛАКОМКА" (ID=23)
в категорию "ЕШЬ ЛЮБИ МОЛИСЬ" (ID=3) с добавлением тега "ЛАКОМКА"
"""
...
class Command(BaseCommand):
    help = 'Переносит статьи из "ЛАКОМКА" в "ЕШЬ ЛЮБИ МОЛИСЬ" с тегом'
    ...
    def handle(self, *args, **options):
        ...
        FROM_ID = 23  # ЛАКОМКА
        TO_ID = 3     # ЕШЬ ЛЮБИ МОЛИСЬ
        TAG_NAME = "ЛАКОМКА"
        
        self.stdout.write(self.style.WARNING('=' * 80))
        self.stdout.write(self.style.WARNING('МИГРАЦИЯ КАТЕГОРИИ "ЛАКОМКА"'))
        self.stdout.write(self.style.WARNING('=' * 80))
